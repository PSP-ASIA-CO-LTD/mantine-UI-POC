...
type ResidentFormValues = Omit<
  Resident,
  'id' | 'createdAt' | 'prefix' | 'guardianId'
> & {
...
export function SalesOrderPage() {
  ...
  const {
    ...
    error: roomError,
  } = useRooms(true); // true for availableOnly
  ...
  const { ref: salesOrderRef, width: salesOrderWidth } =
    useElementSize();
  const isStayDurationTwoCol =
    salesOrderWidth > 0 && salesOrderWidth <= 1192;
  ...
  const [showCreditCardModal, setShowCreditCardModal] =
    useState(false);
    ...
    updateDraft({
      ...
      checkOut:
        checkoutData.checkIn && checkoutData.adjustedDays
          ? new Date(
              checkoutData.checkIn.getTime() +
                (checkoutData.adjustedDays - 1) * 24 * 60 * 60 * 1000,
            ).toISOString()
          : null,
      ...
      status:
        checkoutData.salesOrder?.status === 'paid'
          ? 'paid'
  ...
  useEffect(() => {
    ...
    if (currentDraft.package) {
      ...
      setCheckoutData({
        ...
        checkIn: currentDraft.checkIn
          ? new Date(currentDraft.checkIn)
          : null,
        guardians: currentDraft.guardians,
        primaryContactGuardianId:
          currentDraft.primaryContactGuardianId,
  ...
  const validateGuardian = (
    ...
    if (guardian.firstName.length < 2)
      errors.firstName = 'First name is required';
    if (guardian.lastName.length < 2)
      errors.lastName = 'Last name is required';
    if (guardian.phone.length < 9)
      errors.phone = 'Valid phone is required';
    if (!/^\S+@\S+$/.test(guardian.email))
  ...
  const updateGuardianField = (
    index: number,
    field: keyof GuardianFormValues,
    value: string,
  ) => {
    setGuardianForms((prev) =>
      prev.map((g, i) =>
  ...
  const invoiceTotalsForDisplay = calculateInvoiceTotals(
    invoiceItemsForDisplay,
  );
  ...
  const handleCopyGuardianToResident = () => {
    ...
    residentForm.setValues({
      ...
      emergencyContactName:
        `${firstGuardian.firstName} ${firstGuardian.lastName}`.trim(),
  ...
  const handleAutoFillContactInfo = () => {
    ...
    setGuardianForms([
      {
        ...
        address:
          '123/45 ถ.สุขุมวิท 55 แขวงคลองตันเหนือ เขตวัฒนา กรุงเทพฯ 10110',
        relationship: 'son',
        pays: true,
      ...
      },
      {
        ...
        address:
          '88/9 หมู่บ้านพฤกษา ถ.รัตนาธิเบศร์ อ.เมือง จ.นนทบุรี 11000',
    ...
    residentForm.setValues({
      ...
      medicalConditions:
        'เบาหวานชนิดที่ 2, ความดันโลหิตสูง, ข้อเข่าเสื่อม',
  ...
  const handleContactSubmit = async () => {
    ...
    try {
      ...
      const resident = await saveResident({
        ...
        gender: residentForm.values.gender as
          | 'male'
          | 'female'
          | 'other',
      ...
      const linkedGuardians = await Promise.all(
        savedGuardians.map(async (guardian) => {
          const updated = await updateGuardian(guardian.id, {
  ...
  const handleCreateOrder = async () => {
    const primaryGuardian =
      checkoutData.guardians.find(
        (g) => g.id === checkoutData.primaryContactGuardianId,
      ) || checkoutData.guardians[0];
    ...
    try {
      ...
      checkOut.setDate(
        checkOut.getDate() + checkoutData.adjustedDays - 1,
      );
  ...
  const getStepDescription = (
    ...
    switch (stepIndex) {
      ...
      case 1: // Contact Info
        if (
          checkoutData.guardians.length > 0 &&
          checkoutData.resident
        ) {
      ...
      case 2: // Select Room
        if (checkoutData.room) {
          const extras: string[] = [];
          if (checkoutData.additionalServices.additionalBed)
            extras.push('Bed');
          if (
            checkoutData.additionalServices.specialAmenities.length >
            0
          ) {
            extras.push(
              `+${checkoutData.additionalServices.specialAmenities.length}`,
      ...
      <div className="ds-panel-inline">
        ...
        {step === 0 && (
          <div className="package-list">
            {packages.map((pkg) => {
              const adjustedDays =
                checkoutData.package?.id === pkg.id
                  ? checkoutData.adjustedDays
                  : pkg.duration;
              const endDateLabel = formatStayEndDate(
                ...
                >
                  ...
                  >
                    <div>
                      <Text
                        fw={600}
                        size="lg"
                        data-er-field="SALE_PACKAGE.name"
                      >
                        {pkg.name}
                      </Text>
                      <Text
                        size="sm"
                        c="dimmed"
                        data-er-field="SALE_PACKAGE.duration_days"
                      >
                        {pkg.duration} days • {pkg.services.length}{' '}
                        services
                      </Text>
                    </div>
                    <Group gap="md">
                      <Badge
                        size="lg"
                        color="blue"
                        data-er-field="SALE_PACKAGE.price"
                      >
                  ...
                  <Collapse in={expandedPackage === pkg.id}>
                    ...
                    <Text
                      ...
                      c="dimmed"
                      mb="md"
                      data-er-field="SALE_PACKAGE.description"
                    >
                      {pkg.description}
                    </Text>
                      ...
                      <Grid.Col span={isStayDurationTwoCol ? 6 : 4}>
                        <Stack gap={4}>
                          ...
                          <Text
                            ...
                            fw={700}
                            c="blue"
                            data-er-field="SALES_ORDER.adjusted_price"
                          >
                            ฿
                            {formatCurrency(
                              calculatePrice(pkg, adjustedDays),
                            )}
                          </Text>
                        </Stack>
                      </Grid.Col>
        ...
        {step === 1 && (
          <div className="contact-forms-grid">
            ...
            >
              ...
              <Stack gap="md">
                ...
                <Grid>
                  <Grid.Col span={4}>
                    <DateInput
                      ...
                      value={
                        residentForm.values.dateOfBirth
                          ? new Date(residentForm.values.dateOfBirth)
                          : null
                      }
                      onChange={(val) => {
                ...
                <Checkbox
                  ...
                  {...residentForm.getInputProps(
                    'emergencyContactAddressSameAsResident',
                    { type: 'checkbox' },
                  )}
            ...
            <Card
              ...
              radius="md"
              withBorder
              className="contact-form-card guardians-card"
            >
              <Group justify="space-between" mb="md">
                <div>
                  <Text fw={600} size="lg">
                  ...
                  >
                    <Group justify="space-between" mb="xs">
                      <Text
                        fw={600}
                        size="lg"
                        data-er-field="ROOM.room_number"
                      >
                        Room {room.number}
                      </Text>
                      <Badge
                    ...
                    <Text
                      ...
                      c="blue"
                      mt="xs"
                      data-er-field="ROOM.price_per_day"
                    >
                      +฿{formatCurrency(room.pricePerDay)}/day
                    </Text>
                    {checkoutData.room?.id === room.id && (
                      <Badge color="green" mt="xs" fullWidth>
                        Selected
                      </Badge>
                    )}
                  </Card>
            ...
            <Card padding="lg" radius="md" withBorder>
              ...
              <div className="additional-services-grid">
                ...
                <Paper p="md" withBorder>
                  ...
                  <CheckboxGroup
                    value={
                      checkoutData.additionalServices.specialAmenities
                    }
                    onChange={(value) =>
                      setCheckoutData((prev) => ({
                        ...prev,
                  ...
                  >
                    <Stack gap="xs">
                      ...
                      />
                      <Checkbox
                        value="oxygen_concentrator"
                        label="Oxygen Concentrator (+฿300/day)"
                      />
                      <Checkbox
                        value="air_mattress"
                        label="Air Mattress (+฿200/day)"
                      />
                    </Stack>
                  </CheckboxGroup>
                </Paper>
                ...
                <Paper p="md" withBorder>
                  ...
                  <Stack gap="xs">
                    <Checkbox
                      ...
                      checked={
                        checkoutData.additionalServices
                          .selfProvidePampers ?? false
                      }
                      onChange={(e) => {
                        const checked = e.currentTarget.checked;
        ...
        {step === 3 && checkoutData.invoice && (
          <div className="invoice-step-container">
            ...
            <div className="invoice-a4-paper">
              ...
              <div className="invoice-letterhead">
                ...
                <div className="letterhead-info">
                  ...
                  <Text size="sm" c="dimmed">
                    123 Care Street, Bangkok 10110
                  </Text>
                  <Text size="sm" c="dimmed">
                    Tel: 02-XXX-XXXX | Email:
                    contact@elderlycare.co.th
                  </Text>
                  <Text size="sm" c="dimmed">
              ...
              <Group justify="space-between" mb="xl">
                <div>
                  ...
                  <Text
                    size="sm"
                    c="dimmed"
                    data-er-field="INVOICE.invoice_number"
                  >
                    {checkoutData.invoice.invoiceNumber}
                  </Text>
                </div>
                <Badge
                  size="lg"
                  color={
                    checkoutData.invoice.status === 'paid'
                      ? 'green'
                      : 'yellow'
                  }
                  data-er-field="INVOICE.status"
                >
              ...
              <Grid mb="xl">
                    ...
                    guardianForms
                      ...
                      .map((guardian, idx) => (
                        <Paper
                          ...
                          withBorder
                          mb="xs"
                          className="invoicee-card"
                          data-er-field="INVOICE.guardian_id"
                        >
                          <Text
                            fw={500}
                            data-er-field="INVOICE.guardian_name"
                          >
                ...
                <Grid.Col span={6}>
                  ...
                  <Paper p="sm" withBorder>
                    <Text
                      fw={500}
                      data-er-field="INVOICE.resident_name"
                    >
                      {checkoutData.invoice.residentName}
                    </Text>
                    <Text size="sm" c="dimmed">
                      Room {checkoutData.room?.number}
                    </Text>
                    <Group gap="xs" mt="xs">
                      <Badge size="xs" variant="light">
                        <span data-er-field="SALES_ORDER.check_in">
                          {checkoutData.checkIn?.toLocaleDateString()}
                        </span>{' '}
                        -{' '}
                        <span data-er-field="SALES_ORDER.check_out">
                          {checkoutData.checkIn &&
                          checkoutData.adjustedDays
                            ? new Date(
                                checkoutData.checkIn.getTime() +
                                  (checkoutData.adjustedDays - 1) *
                                    24 *
                                    60 *
                                    60 *
                                    1000,
                      ...
                      <Badge size="xs" variant="outline">
                        {checkoutData.adjustedDays} days
                      </Badge>
                    </Group>
                  </Paper>
                </Grid.Col>
              ...
              <Paper
                ...
                mb="lg"
                className={
                  !checkoutData.package ? 'placeholder-box' : ''
                }
              >
                {checkoutData.package ? (
                  <>
                    <Group justify="space-between" mb="sm">
                      <Text
                        ...
                        data-er-field="SALE_PACKAGE.name"
                      >
                        {checkoutData.package.name}
                      </Text>
                      <Badge
                        color="blue"
                        data-er-field="SALE_PACKAGE.duration_days"
                      >
                        {checkoutData.package.duration} days base
                      </Badge>
                    </Group>
              ...
              <Table mb="xl" className="invoice-table">
                ...
                <Table.Tbody>
                  {invoiceItemsForDisplay.map((item, idx) => (
                    <Table.Tr key={idx} data-er-field="INVOICE_ITEM">
                      <Table.Td data-er-field="INVOICE_ITEM.description">
                        {item.description}
                      </Table.Td>
                      <Table.Td
                        ta="center"
                        data-er-field="INVOICE_ITEM.quantity"
                      >
                        {item.quantity}
                      </Table.Td>
                      <Table.Td
                        ta="right"
                        data-er-field="INVOICE_ITEM.unit_price"
                      >
                        ฿{formatCurrency(item.unitPrice)}
                      </Table.Td>
                      <Table.Td
                        ta="right"
                        data-er-field="INVOICE_ITEM.total"
                      >
                ...
                <Table.Tfoot>
                  <Table.Tr>
                    ...
                    <Table.Td
                      ta="right"
                      data-er-field="INVOICE.subtotal"
                    >
                      ฿
                      {formatCurrency(
                        invoiceTotalsForDisplay.subtotal,
                      )}
                    </Table.Td>
                  </Table.Tr>
                  <Table.Tr>
                    <Table.Td colSpan={3} ta="right">
                      <Text fw={500}>VAT (7%)</Text>
                    </Table.Td>
                    <Table.Td ta="right" data-er-field="INVOICE.tax">
                      ฿{formatCurrency(invoiceTotalsForDisplay.tax)}
                    </Table.Td>
                  </Table.Tr>
                  <Table.Tr className="invoice-total-row">
                    ...
                    <Table.Td
                      ta="right"
                      data-er-field="INVOICE.total"
                    >
                      <Text fw={700} size="xl" c="blue">
                        ฿
                        {formatCurrency(
                          invoiceTotalsForDisplay.total,
                        )}
                      </Text>
                    </Table.Td>
                  </Table.Tr>
              ...
              <Grid mt="xl" className="invoice-signature-section">
                <Grid.Col span={6}>
                  ...
                  <Text size="sm" data-er-field="INVOICE.issued_at">
                    ...
                    onClick={() => {
                      ...
                      navigate('/sales/order/invoice', {
                        state: {
                          ...
                          guardians: checkoutData.guardians,
                          payingGuardians: guardianForms.filter(
                            (g) => g.pays,
                          ),
                          primaryContactGuardianId:
                            checkoutData.primaryContactGuardianId,
                          resident: checkoutData.resident,
                          room: checkoutData.room,
                          additionalServices:
                            checkoutData.additionalServices,
              ...
              <Stack align="center" gap="md">
                <Paper p="lg" withBorder className="qr-placeholder">
                  <Stack align="center" gap="xs">
                    ...
                    <Text size="sm" c="dimmed">
                      Scan to pay ฿
                      {formatCurrency(invoiceTotalsForDisplay.total)}
                    </Text>
                  </Stack>
                </Paper>
                <Text size="sm" c="dimmed">
                  PromptPay ID: 0-1234-56789-01-2
                </Text>
                <Text size="sm" c="dimmed">
                  Elderly Care Facility Co., Ltd.
                </Text>
                <Divider w="100%" />
                <Button
                  fullWidth
            ...
            >
              <Stack gap="md">
                <Text size="sm" c="dimmed">
                  Select bank and confirm payment:
                </Text>
                ...
                >
                  <Stack gap="sm">
                    <Paper
                      p="md"
                      withBorder
                      className={`bank-option ${selectedBank === 'kbank' ? 'selected' : ''}`}
                    >
                      <Radio
                        value="kbank"
                        label={
                    ...
                    <Paper
                      p="md"
                      withBorder
                      className={`bank-option ${selectedBank === 'scb' ? 'selected' : ''}`}
                    >
                      <Radio
                        value="scb"
                        label={
                    ...
                    <Paper
                      p="md"
                      withBorder
                      className={`bank-option ${selectedBank === 'bbl' ? 'selected' : ''}`}
                    >
                      <Radio
                        value="bbl"
                        label={
                    ...
                    <Paper
                      p="md"
                      withBorder
                      className={`bank-option ${selectedBank === 'ktb' ? 'selected' : ''}`}
                    >
                      <Radio
                        value="ktb"
                        label={
                ...
                <Text size="sm" c="dimmed">
                  Account Name: Elderly Care Facility Co., Ltd.
                </Text>
        ...
        {step === 3 && !checkoutData.invoice && (
          <Card padding="xl" radius="md" withBorder ta="center">
            ...
            <Text size="sm" c="dimmed" mb="xl">
              Please complete the previous steps to generate an
              invoice.
            </Text>
              ...
              <div>
                ...
                <Group gap="xs">
                  <Text
                    size="sm"
                    c="dimmed"
                    data-er-field="CONTRACT.contract_number"
                  >
                    Contract #{checkoutData.contract.contractNumber}
                  </Text>
                  <Text size="sm" c="dimmed">
                    •
                  </Text>
                  <Text size="sm" c="dimmed" fw={500}>
                    Order ID: {checkoutData.salesOrder?.id}
                  </Text>
                </Group>
              </div>
              <Badge
                size="lg"
                color={
                  checkoutData.contract.status === 'signed'
                    ? 'green'
                    : 'yellow'
                }
                data-er-field="CONTRACT.status"
              >
            ...
            <Card padding="md" withBorder mb="lg" bg="gray.0">
              <Grid>
                <Grid.Col span={6}>
                  ...
                  <Text fw={500} data-er-field="CONTRACT.start_date">
                    {checkoutData.contract.startDate}
                  </Text>
                </Grid.Col>
                <Grid.Col span={6}>
                  <Text size="sm" c="dimmed">
                  ...
                  <Text fw={500} data-er-field="CONTRACT.end_date">
                    {checkoutData.contract.endDate}
                  </Text>
                </Grid.Col>
                <Grid.Col span={6}>
                  <Text size="sm" c="dimmed">
                  ...
                  {checkoutData.guardians.length > 1 && (
                    <Text size="xs" c="dimmed">
                      +{checkoutData.guardians.length - 1} other
                      guardian(s)
                    </Text>
                  )}
                </Grid.Col>
                <Grid.Col span={6}>
                  ...
                  <Text fw={500} data-er-field="CONTRACT.resident_id">
                    {checkoutData.resident?.firstName}{' '}
                    {checkoutData.resident?.lastName}
                  </Text>
                </Grid.Col>
              </Grid>
              ...
              <Text size="sm" c="dimmed">
                Open the full contract document to review, print,
                sign, and send to all guardians.
              </Text>
            </Card>
            ...
            <Group justify="space-between">
              ...
              <Group gap="sm">
                <Button
                  ...
                  onClick={() => {
                    syncToContext();
                    navigate('/sales/order/contract', {
                      state: { draftId: currentDraft?.id },
                    });
                  }}
                >
                  Open Contract Page
        ...
        {step === 5 && (
          <Card padding="xl" radius="md" withBorder ta="center">
            <IconCheck
              size={64}
              className="ds-empty-state-icon ds-empty-state-icon--success"
            />
            <Title order={2} mb="sm">
              Order Complete!
            </Title>
            ...
            <Card padding="lg" withBorder mb="xl" ta="left">
              ...
              <Stack gap="sm">
                ...
                <Group gap="sm">
                  <IconReceipt size={20} />
                  <Text size="sm">
                    Receipt #{checkoutData.invoice?.invoiceNumber} is
                    ready
                  </Text>
                </Group>
                <Group gap="sm">
                  ...
                  <Text size="sm">
                    Check-in: {checkoutData.salesOrder?.checkIn} |
                    Check-out: {checkoutData.salesOrder?.checkOut}
                  </Text>
                </Group>
              </Stack>
            </Card>
            ...
            >
              <Stack gap="xs">
                <Text size="sm">
                  1. Please arrive at the facility on the check-in
                  date by 10:00 AM
                </Text>
                <Text size="sm">
                  2. Bring the resident's ID card and any relevant
                  medical documents
                </Text>
                <Text size="sm">
                  3. Prepare personal belongings (clothes, toiletries,
                  medications)
                </Text>
                <Text size="sm">
                  4. Our staff will provide a facility tour and
                  introduction
                </Text>
              </Stack>
            </Card>
