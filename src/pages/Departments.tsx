...
const tokenColor = (token: string) => `var(${token})`;

const DEPARTMENT_COLOR_PRESETS = [
    tokenColor('--mantine-color-blue-6'),
    tokenColor('--mantine-color-green-8'),
    tokenColor('--mantine-color-orange-9'),
    tokenColor('--mantine-color-grape-8'),
    tokenColor('--mantine-color-cyan-6'),
    tokenColor('--mantine-color-lime-9'),
    tokenColor('--mantine-color-violet-9'),
    tokenColor('--mantine-color-yellow-8'),
    tokenColor('--mantine-color-indigo-9'),
    tokenColor('--mantine-color-teal-9'),
    tokenColor('--mantine-color-pink-7'),
    tokenColor('--mantine-color-dark-4'),
    tokenColor('--mantine-color-dark-3'),
    tokenColor('--mantine-color-red-9'),
    tokenColor('--mantine-color-green-9'),
    ...
    tokenColor('--mantine-color-yellow-7'),
    tokenColor('--mantine-color-blue-4'),
    tokenColor('--mantine-color-gray-6'),
];
...
const extractFallbackColor = (value: string) => {
    const trimmed = value.trim();
    if (!trimmed.startsWith('var(')) return trimmed;

    const commaIndex = trimmed.indexOf(',');
    const closeIndex = trimmed.lastIndexOf(')');
    if (commaIndex === -1 || closeIndex === -1 || closeIndex <= commaIndex) return trimmed;
    return trimmed.slice(commaIndex + 1, closeIndex).trim();
};

const splitVarArgs = (value: string): [string, string] => {
    let depth = 0;
    for (let i = 0; i < value.length; i += 1) {
        const ch = value[i];
        if (ch === '(') depth += 1;
        if (ch === ')') depth -= 1;
        if (ch === ',' && depth === 0) return [value.slice(0, i), value.slice(i + 1)];
    }
    return [value, ''];
};
...
const DEPARTMENT_COLOR_LABEL_BY_VALUE = new Map<string, string>(
    DEPARTMENT_COLOR_OPTIONS.flatMap((option) => {
        ...
        return [...new Set(keys)].map((key) => [key, option.label] as const);
    }),
);
